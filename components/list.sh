...
function usage() {
    ...
    echo "${0##*/}:                    ${bold}[--vnd] [--fiat <CURRENCY>]${reset}"
    ...
    echo "${0##*/}: Example: ${0##*/} -t -n 7 -vn ...  uses Tor, prints market info of top 7 crypto currencies, "
    echo "${0##*/}:                              uses VND for prices"
    echo "${0##*/}: Example: ${0##*/} -vn  ...  shortcut for -f VND, uses VND for prices"
    ...
    echo "${0##*/}: Example: ${0##*/} -t -f VND -l btc,sc,btcd -p 100  ...  lists BTC, SC and BTCD "
    echo "${0##*/}:                              by searching in the top 100 crypto currencies,"
    echo "${0##*/}:                              communicates via Tor and uses VNDs for prices"
    echo "${0##*/}: Example: ${0##*/} -t -f VND -i bitcoin-cash, ethereum-classic  ...  lists BCH and ETC "
    ...
    echo "${0##*/}:    VND: Use VND as fiat currency instead of default USD, shortcut for -f VND [type: flag]"
